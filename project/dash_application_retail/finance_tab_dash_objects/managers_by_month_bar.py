...
def managers_by_month_func(df, retail_customer_select, manager):
    ...
    fig = go.Figure(go.Bar(
        x=df_groupped['month_first_date'],
        y=df_groupped['amount'] / 1000000,
        text=df_groupped['amount'] / 1000000,
    ...
    fig.update_layout({
        ...
        "bargap": 0.30,
        # 'barmode': 'stack',
        "title": f"Продажи менеджера {manager}",
        # 'legend': {
        #     'orientation': "h",
        #     'yanchor': "bottom",
        #     'y': 1.08,
    ...
    fig.update_traces(
        ...
        hovertemplate='<extra></extra>%{x}:<br>%{text:.1f} млн.руб',
    )
    return fig
