...
def credit_calendar_to_bd_func(data_input):
    ...
    if data_input == '1с_api':
        ...
        test_bank = 'СМП БАНК АО'
        test_bank = 'ЕАБР'
        # test_bank = 'ФК ОТКРЫТИЕ ПАО БАНК'

        test_bank_agreement_calendar_df = all_agreements_df.loc[all_agreements_df['creditor']==test_bank]


            # output_object_list.append(f'{agreement_start_datetime} dddd {agreement_finish_datetime} ffff')
        ...
        try:
            ...
            status_text = "не получилось создать таблицу all_agreements_df"
        ...
        # data = refunded_payments_df.to_dict('records')
        # credit_datatable = dash_table.DataTable(data=data, )

        # data_2 = test_bank_agreement_calendar_df.to_dict('records')
        # credit_datatable_2 = dash_table.DataTable(data=data_2, )
